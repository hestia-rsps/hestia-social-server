...
class MessagePublicQuickChatHandler : PlayerMessageHandler<MessagePublicQuickChat>() {

    override fun handle(ctx: ChannelHandlerContext, player: Player, message: MessagePublicQuickChat) {
        var (script, file, data) = message
            ...
            0 -> {//Public
                World.players.all.forEach {
                    if(player.index != null) {
                        it.send(ChatPublicQuickChat(player.index!!, 0x8000, player.rights, file, data))
                    }
                }
            }
            1 -> {//Friends chat
