...
object Huffman {
    ...
    fun init() {
        val cacheIndex = Cache.getIndex(10) ?: return
        val huffman = cacheIndex.getFile(cacheIndex.getArchiveId("huffman"))
     ...
     */
    fun compress(message: String, builder: Packet.Builder) {
        try {
            //Format the message
     ...
     * @param builder The packet to write the compressed data too
     */
    private fun compress(message: ByteArray, builder: Packet.Builder) {
        try {
            if(HUFFMAN_MASKS == null) {
                return
            }
